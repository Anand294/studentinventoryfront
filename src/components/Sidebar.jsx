...
function NavGroup({ item, onNavigate }) {
  ...
  if (!item.children)
    return (
      <NavLink
        ...
        onClick={onNavigate}
        className={({ isActive }) =>
          `link primary-link ${isActive ? "active" : ""}`
        }
      >
        <Icon name={item.icon} />
  ...
  return (
    <div className="menu-group">
      ...
      <div className={`submenu ${open ? "submenu-open" : ""}`}>
        <div>
...
export default function Sidebar({ open, onClose }) {
  const closeOnMobileOnly = () => {
    if (window.innerWidth <= 950) onClose();
  };
  return (
